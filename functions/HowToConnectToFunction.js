...
const UpdateNotification = async (doc_id) => {
  const res = await functions.createExecution(
    FUNCTION1_ID,
    JSON.stringify({ doc_id: doc_id }),
    false,
    "/facilitator/notification/put",
    ExecutionMethod.PUT
  );
  if (res.responseStatusCode === 200) {
    const rawData = JSON.parse(res.responseBody);
...
const VerifyAllUsers = async () => {
  ...
  if (res.responseStatusCode === 200) {
    ...
    throw new Error("Verify all users failed");
  }
};
