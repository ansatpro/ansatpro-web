      ...
      const enrichedFeedbacks = await Promise.all(
        feedbacks.documents.map(async (fb) => {
          ...
          await Promise.all(aiItems.map(async (item) => {
            try {
              const assessment = await adminDatabase.getDocument(
                process.env.DB_ID,
                process.env.ASSESSMENT_ITEMS_ID,
                [Query.equal('item_id', [item.item_id])]
              );
              const description = assessmentResult.documents[0]?.description || "Description not found";
