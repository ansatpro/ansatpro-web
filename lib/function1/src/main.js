   ...
   if (req.path === "/facilitator/get/studentsWithAllDetails") {
    switch (req.method) {
      case "GET":
        ...
        log(student_document_ids)

        // get student feedback from preceptor
        const feedbackPromises = student_document_ids.map(id =>
          db.getDocument(DB_ID, COLLECTION_ID_PRECEPTOR_FEEDBACKS, id)
        );
        
        const allFeedbackResponses = await Promise.all(feedbackPromises);
        
        log(allFeedbackResponses); // log all responses at once
