   ...
   if (req.path === "/facilitator/get/studentsWithAllDetails") {
    switch (req.method) {
      case "GET":
        ...
        const ids = allDocuments_students.map(student => student.$id);
        log(ids)
        ...
        for (let i = 0; i < student_document_ids.length; i += chunksize2) {
          chunks2.push(student_document_ids.slice(i, i + chunksize2));
        }
        ...
        for (const chunk of chunks2) {
          const res = await db.listDocuments(
            ...
            [Query.equal("student_document_id", chunk)]
          );
          allDocuments2.push(...res.documents);
        }
