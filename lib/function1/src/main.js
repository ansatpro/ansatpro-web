...
export default async ({ req, res, log, error }) => {
  ...
  function createAIPrompt(feedbackData) {
    const studentName = feedbackData[0]?.content?.match(/Penny Wong/)
      ? 'Penny Wong'
      : 'Student'; // Assuming the student's name is in the content
    ...
    Provide the summary with bullet points for each section. For each description, include a score from 0 to 5 (or N/A if not applicable).
  `;
  }
