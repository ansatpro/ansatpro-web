      ...
      case "POST":
        try {
          ...
          try {
            ...
            log(`Failed to update user verification status: ${userError.message}`);
  ...
  if (req.path === "/facilitator/get/studentsWithAllDetails") {
    ...
    switch (req.method) {
      case "GET":
        ...
        if (!created_by) {
          throw new Error("Missing 'created_by' in query parameters.");
        }
  
        log(created_by);
        return context.res.json({
          message: "Successfully retrieved 'created_by'.",
