...
// Get All Students with preceptor_feedback
const GetAllStudentsWithPreceptorFeedback = async () => {
  const user = await account.get();
  const res = await functions.createExecution(
    FUNCTION1_ID,
    JSON.stringify({ created_by: user.$id }),
...
export {
  ...
  GetAllStudents,
  DeleteStudent,
  GetAllStudentsWithPreceptorFeedback,
};
