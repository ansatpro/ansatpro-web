...
export default async ({ req, res, log }) => {
  ...
  const userDatabase = new Databases(userClient); // 👈 Rename this
  ...
  const adminDatabase = new Databases(adminClient); // 👈 Explicit and correct
  ...
  try {
    ...
    if (action === 'getStudentsList') {
      ...
      const metadataDoc = await adminDatabase.getDocument(
        process.env.DB_ID,
        process.env.USERS_METADATA_COLLECTION_ID,
      ...
      if (role === 'preceptor') {
        ...
        students = await adminDatabase.listDocuments(
          process.env.DB_ID,
          process.env.STUDENTS_COLLECTION_ID
        );
    ...
    if (action === 'addMetadata') {
      try {
        log("🟢 Starting addMetadata action...");

        log("🚨 Verifying which client is being used");
        log("adminDatabase client headers:", adminDatabase.client.headers);
        ...
        try {
          log("🔍 Attempting to get existing metadata document...");
          existingDoc = await adminDatabase.getDocument(
            process.env.DB_ID,
            process.env.USERS_METADATA_COLLECTION_ID,
            user.$id
          );
          log("✅ Document found. Proceeding to update.");
        } catch (err) {
          if (err.code === 404) {
            log("❗ Document not found. Will create a new one.");
          } else {
            log("❌ Failed to get document. Error:", JSON.stringify(err));
        ...
        if (existingDoc) {
          ...
          log("📄 Creating new metadata document...");
          const created = await adminDatabase.createDocument(
            process.env.DB_ID,
            process.env.USERS_METADATA_COLLECTION_ID,
            user.$id,
        ...
        log("🔥 ERROR in addMetadata:", JSON.stringify(err, null, 2));
        return res.json({
          status: "error",
