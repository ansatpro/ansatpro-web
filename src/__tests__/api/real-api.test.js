...
afterAll(async () => {
  try {
    // Clean up test data
    if (testFeedbackId) {
      await databases.deleteDocument(DATABASE_ID, FEEDBACK_COLLECTION_ID, testFeedbackId);
    }
    if (testNotificationId) {
      await databases.deleteDocument(DATABASE_ID, NOTIFICATIONS_COLLECTION_ID, testNotificationId);
    }
...
describe('Authentication API', () => {
  test('Create and delete sessions', async () => {
    ...
    try {
      ...
      // Delete the session
      const deleteResponse = await fetch(`${API_BASE_URL}/account/sessions/current`, {
        method: 'DELETE',
        headers: {
          'X-Appwrite-Project': process.env.NEXT_PUBLIC_APPWRITE_PROJECT_ID,
  ...
  test('List students with filters', async () => {
    ...
    try {
      ...
      if (testStudentId) {
        ...
        const found = searchResponse.documents.some(doc => doc.$id === testStudentId);
...
describe('Feedback API', () => {
  test('Create and retrieve feedback', async () => {
    ...
    try {
      ...
      expect(createResponse.student_document_id).toBe(feedbackData.student_document_id);
      ...
      const found = listResponse.documents.some(doc => doc.$id === testFeedbackId);
      expect(found).toBe(true);
    } catch (error) {
      console.error('Test failed:', error);
      throw error;
...
describe('Notification API', () => {
  test('Create, fetch, and mark notification as read', async () => {
    ...
    try {
      ...
      const found = listResponse.documents.some(doc => doc.$id === testNotificationId);
      expect(found).toBe(true);
      ...
      const foundAfterUpdate = listAfterUpdate.documents.some(doc => doc.$id === testNotificationId);
