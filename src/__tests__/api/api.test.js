...
describe('Authentication API', () => {
  test('Guest endpoint should be accessible without authentication', async () => {
    ...
    const response = await fetch(`${API_BASE_URL}/functions/${FUNCTION_ENDPOINT}/executions`, {
      method: 'POST',
      headers: {
  ...
  test('Protected endpoints should require authentication', async () => {
    ...
    const response = await fetch(`${API_BASE_URL}/functions/${process.env.NEXT_PUBLIC_FN_USER_METADATA}/executions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
describe('Feedback API', () => {
  test('Can post feedback', async () => {
    ...
    const feedbackData = {
      ...
      feedback_text: 'Student is doing well. Shows great progress in clinical skills.',
  ...
  test('Can get AI summary', async () => {
    ...
    const mockGetAISummary = async (studentId) => {
      ...
      // For this example, we'll mock the response
      return {
        summary: 'Student shows consistent progress across multiple clinical areas. Strengths include patient communication and critical thinking. Areas for improvement include documentation and time management.',
        feedbacks: [
          { date: '2023-03-10', text: 'Good progress in patient communication' },
